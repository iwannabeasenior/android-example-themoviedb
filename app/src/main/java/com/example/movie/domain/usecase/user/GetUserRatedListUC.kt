...
class GetUserRatedListUC @Inject constructor(private val userRepo: UserRepo, private val mapper: MovieMapper) {
    suspend fun execute(accountId: Int, sessionId: String): Result<List<Movie>> {
        return handleResponse(userRepo.getUserRatedList(accountId, sessionId), mapper::mapListMovie)
    }
}
